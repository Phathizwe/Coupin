// src/scripts/deployOptimizedIndexes.fixed.js
const { execSync } = require('child_process');
const fs = require('fs');
const path = require('path');

/**
 * Script to deploy optimized Firestore indexes using local firebase-tools
 */
function deployOptimizedIndexes() {
  try {
    ...
    // Use local firebase-tools from node_modules
    const firebaseBin = path.join(__dirname, '..', '..', 'node_modules', '.bin', 'firebase');
    
    // Deploy indexes to Firebase using local binary
    execSync(`${firebaseBin} deploy --only firestore:indexes`, { stdio: 'inherit' });

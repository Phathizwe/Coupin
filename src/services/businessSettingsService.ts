  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      return {
        ...
        address: '',
        website: '',
        industry: '' // Added default empty industry
      };
    }
  } catch (error) {
...
export const updateBusinessProfile = async (businessId: string, profile: Partial<BusinessProfile>): Promise<void> => {
  try {
    const docRef = doc(db, 'businesses', businessId, 'settings', 'profile');
...
// DEVELOPMENT MODE: Convert file to data URL instead of Firebase Storage

    ...
    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
      <div 
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">{store.industry || 'General'}</p>
        </div>
      </div>
      <div className="p-4">
        <p className="text-sm text-gray-600 mb-3">{store.description || 'No description available'}</p>
        <p className="text-xs text-gray-500 mb-4">{store.address || 'No address provided'}</p>
            ...
            {couponCount > 0 ? (
              <>
                <span className="font-medium" style={{ color: secondaryColor }}>
                  {couponCount}
                </span> {couponCount === 1 ? 'coupon' : 'coupons'} available
              </>
            ) : (
      ...
      <div className="px-4 py-3 bg-gray-50 border-t border-gray-200 flex justify-between">
        ...
        <button
          ...
          style={{ 
            backgroundColor: couponCount > 0 ? primaryColor : undefined,
            outlineColor: primaryColor
          }}
          disabled={couponCount === 0}
          onClick={couponCount > 0 ? handleViewCoupons : undefined}
        >
          View Coupons
        </button>
      </div>

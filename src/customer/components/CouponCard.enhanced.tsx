...
interface CouponCardProps {
  ...
  compact?: boolean; // Original compact prop
  viewMode?: 'default' | 'simple'; // New viewMode prop
}
...
const CouponCard: React.FC<CouponCardProps> = ({ 
  ...
  // Use either the explicit compact prop or derive it from viewMode
  const isCompact = compact || viewMode === 'simple';
  ...
  return (
    <div className={`bg-white rounded-lg shadow-md overflow-hidden border border-gray-200 hover:shadow-lg transition-shadow duration-300 ${isCompact ? 'compact-coupon' : ''}`}>
      {/* Coupon header with business info - now styled with the business colors */}
      <div 
        className={`p-3 border-b flex items-center ${isCompact ? 'p-2' : 'p-3'}`}
      ...
      >
        {coupon.businessLogo ? (
          <img 
            ...
            className={`rounded-full mr-3 object-cover border-2 ${isCompact ? 'w-8 h-8' : 'w-10 h-10'}`}
          ...
          <div 
            className={`rounded-full flex items-center justify-center mr-3 text-white ${isCompact ? 'w-8 h-8' : 'w-10 h-10'}`}
        ...
        <div>
          ...
          <p className={`text-gray-500 ${isCompact ? 'text-xs' : 'text-sm'}`}>Valid until {formatDate(coupon.endDate)}</p>
        </div>
      </div>
      
      {/* Coupon content */}
      <div className={isCompact ? 'p-3' : 'p-4'}>
        <h2 className={`font-bold text-gray-800 mb-2 ${isCompact ? 'text-base' : 'text-lg'}`}>{coupon.title}</h2>
        {!isCompact && <p className="text-gray-600 text-sm mb-3">{coupon.description}</p>}

        <div className={`bg-gray-100 rounded-md mb-3 flex justify-between items-center ${isCompact ? 'p-2' : 'p-3'}`}>
          <div className={`font-mono font-semibold ${isCompact ? 'text-base' : 'text-lg'}`}>{coupon.code}</div>
          <button 
        ...
        <div className="flex items-center justify-between">
          ...
          {isRedeemed ? (
            ...
            <button
              ...
              className={`px-4 py-2 text-white rounded-md text-sm font-medium hover:opacity-90 disabled:opacity-50 ${isCompact ? 'px-3 py-1' : 'px-4 py-2'}`}
              style={{ backgroundColor: primaryColor }}
            >
              {isRedeeming ? 'Redeeming...' : isCompact ? 'Redeem' : 'Redeem Now'}
            </button>
          )}
        </div>
      </div>

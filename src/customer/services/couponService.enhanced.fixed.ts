...
export const fetchCustomerCoupons = async (
  ...
  try {
    ...
    let linkedCustomer;
    try {
      linkedCustomer = await findCustomerByUserId(userId);
    } catch (error) {
      console.warn('Error finding linked customer:', error);
      // Continue without linked customer
    }
    ...
    try {
      couponIds = await fetchCouponIds(idsToSearch, pageSize);
    } catch (error) {
    ...
    let customerCoupons: Coupon[] = [];
    try {
      customerCoupons = await fetchCouponDetails(couponIds);
    } catch (error) {

...
export const fetchCouponIds = async (idsToSearch: string[], pageSize: number): Promise<string[]> => {
  ...
  // Check the couponDistributions collection for both user ID and customer ID
  const distributionDocs = await fetchDistributionDocs(idsToSearch);
  console.log('Total distributions found:', distributionDocs.length);
  
  // Check the customerCoupons collection for both user ID and customer ID
  const customerCouponDocs = await fetchCustomerCouponDocs(idsToSearch, pageSize);
  ...
  if (customerPhone) {
    phoneLinkedDocs = await fetchPhoneLinkedCoupons(customerPhone);

...
export const CurrencyProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    setTimeout(() => {
      ...
      document.dispatchEvent(new CustomEvent('currency-changed', { 
        detail: { 
          ...
          isExplicit: true,
          preserveChoice: true // Add preserveChoice flag to ensure the event is processed
        } 
      }));
    }, 300);
  };
    ...
    if (!userExplicitlySelectedCurrency) {
      ...
      if (defaultCurrencyForRegion) {
        ...
        setTimeout(() => {
          document.dispatchEvent(new CustomEvent('currency-changed', { 
            detail: { 
              currency: defaultCurrencyForRegion,
              preserveChoice: true // Add preserveChoice flag to ensure the event is processed
            } 
          }));
        }, 100);
  ...
  useEffect(() => {
    ...
    const handleCurrencyChange = (event: any) => {
      ...
      // Modified condition to ensure we always process the event when the currency changes
      // Only use the condition if we're not sure about updating the state
      if (event.detail.currency !== currency) {
        console.log('[CurrencyContext] Updating currency state from event');

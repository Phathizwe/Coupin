...
interface DetailedLoyaltyActionsProps {
  ...
  onScanClick: () => void;
  onSendCouponClick: () => void; // We'll keep the same prop name for compatibility
  onRewardClick: () => void;
}
...
const DetailedLoyaltyActions: React.FC<DetailedLoyaltyActionsProps> = ({
  program,
  onScanClick,
  onSendCouponClick, // This will now be used for the Invite Member action
  onRewardClick
}) => {
  if (!program) {
    return null;
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        >
          <div className="flex items-center mb-3">
            ...
            <div className="ml-3">
              <h3 className="font-medium text-indigo-900">Invite Member</h3>
              <p className="text-xs text-indigo-700">Add customers to program</p>
            </div>
          </div>
          <p className="text-sm text-indigo-800">
            Invite customers to join your loyalty program and start earning rewards
          </p>

...
interface CouponMascotGuideProps {
  couponsCount: number;
  activeFilter: string;
  isSearching: boolean;
  customTip?: Tip;
...
const CouponMascotGuide: React.FC<CouponMascotGuideProps> = ({
  couponsCount,
  activeFilter,
  isSearching,
  customTip
  ...
  const handleMascotClick = () => {
    ...
    setTimeout(() => setMascotInteractive(false), 1000);
  };
    ...
    <div className="fixed bottom-4 right-4 z-20">
      <div className="relative">
        ...
        <AnimatePresence>
          {showTip && currentTip && (
            <motion.div
              className="absolute bottom-16 right-0 bg-white rounded-xl p-4 shadow-lg max-w-xs"
        ...
        {/* Mascot button */}
        <motion.button
          className="w-14 h-14 bg-gradient-to-br from-amber-400 to-amber-500 rounded-full shadow-lg flex items-center justify-center text-2xl"
          onClick={handleMascotClick}
          whileHover={{ scale: 1.1, rotate: [0, -5, 5, -5, 0] }}
          whileTap={{ scale: 0.9 }}
          animate={mascotInteractive ? {
            rotate: [0, -15, 15, -15, 0],
            scale: [1, 1.2, 1]
          } : {}}
...
export default CouponMascotGuide;
...
// Enhanced color schemes for different coupon types
...
interface EnhancedCouponCardProps {
  coupon: CouponData;
...
const EnhancedCouponCard: React.FC<EnhancedCouponCardProps> = ({
  coupon,
  onSelect,
  onCopy,
  selected = false
  ...
  const colorScheme = coupon.colorScheme || enhancedColorSchemes[coupon.type];
  ...
  return (
    <motion.div
      className={`relative rounded-xl overflow-hidden shadow-sm border ${selected ? 'border-amber-400' : 'border-gray-200'
    ...
    >
      ...
      {/* Main card content with enhanced gradients */}
      <div className={`${colorScheme.secondary} p-4`}>
        ...
        {/* Usage stats and expiry with enhanced visualization */}
        <div className="mt-4">
          ...
          {/* Enhanced usage progress bar with animation */}
          {coupon.maxUses > 0 && (
      ...
      {selected && (
        ...
        >
          <motion.div
            className="absolute top-1 left-1 w-4 h-4 bg-amber-400 rounded-full flex items-center justify-center"
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
